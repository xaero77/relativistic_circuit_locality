...
- coherent-state overlap 기반 partial trace 에 더해, mode별 thermal occupation(`environment_temperature` 또는 `thermal_occupations`), bath spectral density(`bath_spectral_density`), 일반 Lindblad jump operator(`lindblad_operators`, `lindblad_rates`, `lindblad_time`), non-Markovian memory kernel(`memory_kernel`, `memory_strength`)과 colored noise correlation(`colored_noise_correlation`)을 포함해 purity/coherence 를 계산하는 `DecoherenceResult`, `compute_decoherence_model` 추가
