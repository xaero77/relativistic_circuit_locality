...
- **decoherence 모델 단순화 (추가 개선, 여전히 부분 해소)**: `compute_decoherence_model`이 vacuum coherent-state overlap 위에 mode별 thermal Bose occupation `n_k`를 곱한 suppression factor `exp[-1/2 Σ_k (2 n_k + 1)|Δα_k|²]`를 적용하고, branch basis reduced density matrix `ρ`에 대해 일반 Lindblad master equation `dρ/dt = Σ_i γ_i (L_i ρ L_i† - 1/2 {L_i†L_i, ρ})`를 RK4 로 적분한다. 여기에 `bath_spectral_density(ω)`로 mode별 thermal weight 를 재가중하고, `memory_kernel`과 `memory_strength`로 시간 비국소 convolution memory term 을 Lindblad 진화에 더하며, `colored_noise_correlation`의 적분 norm 으로 비백색 환경 잡음의 추가 suppression 을 반영한다. 추가로 `auto_lindblad_from_bath=True`일 때는 branch-basis 에너지 gap과 spectral density, 온도에서 emission/absorption jump operator 를 자동 생성해 KMS detailed-balance ratio `Γ_up/Γ_down = e^{-βΔE}`를 만족하는 surrogate bath coupling 을 구성하고, `detailed_balance_deviation`으로 그 편차를 보고한다. 또한 `auto_lamb_shift_from_bath=True`일 때는 같은 spectral density 로부터 principal-value surrogate 적분을 수행해 diagonal Lamb-shift Hamiltonian 과 `renormalized_transition_energies`를 구성함으로써 bath-induced self-energy renormalization 을 동역학에 포함한다. 나아가 `auto_bath_dressing_from_bath=True`일 때는 branch-basis coherence 와 bath spectral density 에서 off-diagonal bath dressing Hamiltonian `bath_dressing_matrix`를 생성해 비대각 self-energy mixing 을 추가한다. 추가로 `influence_phase_strength`와 `non_gaussian_cumulant_strength`가 주어지면 time-grid 위에서 bath spectral density 및 colored noise correlation 을 적분해 influence-functional surrogate phase 와 비가우시안 cumulant 보정을 `influence_phase_matrix` 및 `non_gaussian_cumulant_norm`으로 구성하고, `influence_iterations`/`influence_tolerance`/`influence_relaxation`으로 자기일관 influence resummation 을 반복해 `influence_residual`이 수렴할 때까지 비섭동적 surrogate dressing 을 갱신한다. 따라서 기존의 유한 개 mode + Markovian Lindblad surrogate 에서 더 나아가 spectral-density-shaped bath, KMS-consistent thermal transition, bath-induced Lamb shift, off-diagonal bath dressing, non-Markovian/colored-noise 및 self-consistent multitime influence-functional surrogate open-system 진화를 직접 모사한다. 다만 완전한 exact open QFT influence functional 자체와 연속 함수공간에서의 닫힌형 해는 아직 남아 있다.
