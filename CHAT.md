...
- 예제 실행은 `python -m relativistic_circuit_locality.examples {core|field|research}`로 분리되어 있고, 위상 비교, branch pair phase 분해, finite-width wavepacket, displacement/coherent-state, surrogate/PDE/closure 계열 예제도 시나리오별로 나뉘어 실행된다
