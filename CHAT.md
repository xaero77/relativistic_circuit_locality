...
- **microcausality 판정이 자명 (해소)**: `evaluate_microcausality_commutator`가 branch segment 쌍에 대해 Pauli-Jordan commutator 를 Gauss-Legendre 이중 적분으로 평가한다. massive Klein-Gordon tail `-m J1(mτ)/(4πτ)`와 수치적으로 regularized 한 light-cone shell 을 함께 반영하므로, 단순 0/1 판정 대신 spacelike support 에서는 0, timelike/null support 에서는 비영 commutator norm 을 반환한다.
- **텐서 구조 근사 수준 (해소, exact QFT 는 아님)**: `compute_tensor_mediated_phase_matrix`가 branch 분리 방향에 대한 transverse/longitudinal projector 를 구성해 vector current-current contraction 을 평가하고, gravity 채널에는 같은 projector 위의 traceless spatial stress contraction 을 추가한다. `gauge_scheme={"projected","landau","feynman","coulomb","unitary"}` 와 `gauge_parameter`로 separation-dependent nonlocal longitudinal weight 를 갖는 gauge-fixed surrogate propagator 를 선택할 수 있고, `vertex_resummation={"none","geometric","pade","exponential"}` 및 `vertex_strength`로 국소 phase density 기반 4D vertex resummation surrogate 를 적용한다. 추가로 `ghost_mode={"none","faddeev_popov","brst"}` 와 `ghost_strength`가 Landau gauge slice 로 수렴하는 ghost phase 와 BRST-compensated vector phase 를 계산하고, `dyson_schwinger_mode={"none","rainbow","ladder","coupled"}` 및 관련 반복 파라미터가 self-energy kernel 기반의 self-consistent tensor dressing 을 반복해 dressed vector/gravity channel 을 반환한다. 따라서 단순 `1 ± v_a·v_b/c²` 평균 보정이 아니라, 평행/수직 운동 방향, gauge fixing, ghost cancellation, BRST residual, infinite-order surrogate self-dressing 을 함께 반영한다. 다만 완전한 연속 함수공간 BRST cohomology 와 자외선 재규격화까지 포함한 exact Dyson-Schwinger functional solution 은 아직 연구형 과제로 남는다.
- **유한차분 PDE solver (해소, 수치 고도화 여지 있음)**: `solve_finite_difference_kg`가 tensor-product `spatial_points`를 자동 인식해 Cartesian 3D leapfrog 진화를 수행하고, coarse `time_slices` 사이에서는 Courant 수에 맞춰 adaptive substepping 을 적용한다. 여기에 `adaptive_mesh_refinement_rounds`와 `adaptive_mesh_radius_factor`로 source worldline 주변 구간을 midpoint 기반으로 세분화하는 adaptive mesh refinement 를 적용하고, `remeshing_metric` 3x3 SPD tensor 로 축별 metric weight 를 주어 anisotropic metric-adaptive remeshing 을 수행한다. 또한 `boundary_level_set` callable 이 주어지면 level-set 형태의 임의 곡면 경계에 대해 active/inactive grid 를 나누는 데서 멈추지 않고, 절단된 face aperture 와 cell volume fraction 을 계산해 flux-form cut-cell 경계 적분으로 Laplacian 을 보정한다. `stencil_order=4`에서는 1D 5-point 및 다차원 축별 4차 정확도 Laplacian 을 선택할 수 있으며, `FiniteDifferencePdeResult`는 `grid_shape`, `spatial_dimension`, `effective_time_step`, `substeps_per_interval`뿐 아니라 `stencil_order`, `refinement_rounds`, `boundary_geometry`, `active_point_mask`, `remeshing_metric`, `cell_volume_fractions`, `face_apertures`도 반환해 내부 격자 구조를 추적한다.
- **decoherence 모델 단순화 (대부분 해소, exact influence functional 은 아님)**: `compute_decoherence_model`이 vacuum coherent-state overlap 위에 mode별 thermal Bose occupation `n_k`를 곱한 suppression factor `exp[-1/2 Σ_k (2 n_k + 1)|Δα_k|²]`를 적용하고, branch basis reduced density matrix `ρ`에 대해 일반 Lindblad master equation `dρ/dt = Σ_i γ_i (L_i ρ L_i† - 1/2 {L_i†L_i, ρ})`를 RK4 로 적분한다. 여기에 `bath_spectral_density(ω)`로 mode별 thermal weight 를 재가중하고, `memory_kernel`과 `memory_strength`로 시간 비국소 convolution memory term 을 Lindblad 진화에 더하며, `colored_noise_correlation`의 적분 norm 으로 비백색 환경 잡음의 추가 suppression 을 반영한다. 추가로 `auto_lindblad_from_bath=True`일 때는 서로 다른 branch-basis level pair 전체의 에너지 gap, spectral density, 온도에서 emission/absorption jump operator 를 자동 생성해 각 전이에 대해 KMS detailed-balance ratio `Γ_up/Γ_down = e^{-βΔE}`를 만족하는 surrogate bath coupling 을 구성하고, `detailed_balance_deviation`으로 그 편차를 보고한다. 또한 `auto_lamb_shift_from_bath=True`일 때는 같은 spectral density 로부터 principal-value surrogate 적분을 수행해 diagonal Lamb-shift Hamiltonian 과 `renormalized_transition_energies`를 구성함으로써 bath-induced self-energy renormalization 을 동역학에 포함한다. 나아가 `auto_bath_dressing_from_bath=True`일 때는 branch-basis coherence 와 bath spectral density 에서 off-diagonal bath dressing Hamiltonian `bath_dressing_matrix`를 생성해 비대각 self-energy mixing 을 추가한다. 추가로 `influence_phase_strength`와 `non_gaussian_cumulant_strength`가 주어지면 time-grid 위에서 bath spectral density 및 colored noise correlation 을 적분해 influence-functional surrogate phase 와 비가우시안 cumulant 보정을 `influence_phase_matrix` 및 `non_gaussian_cumulant_norm`으로 구성하고, `influence_iterations`/`influence_tolerance`/`influence_relaxation`으로 자기일관 influence resummation 을 반복해 `influence_residual`이 수렴할 때까지 비섭동적 surrogate dressing 을 갱신한다. 따라서 기존의 유한 개 mode + Markovian Lindblad surrogate 에서 더 나아가 spectral-density-shaped bath, pair-resolved KMS-consistent thermal transition, bath-induced Lamb shift, off-diagonal bath dressing, non-Markovian/colored-noise 및 self-consistent multitime influence-functional surrogate open-system 진화를 직접 모사한다. 다만 완전한 exact open QFT influence functional 자체와 연속 함수공간에서의 닫힌형 해는 아직 남아 있다.
