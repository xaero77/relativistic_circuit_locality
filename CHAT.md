...
- **decoherence 모델 단순화 (추가 개선, 여전히 부분 해소)**: `compute_decoherence_model`이 vacuum coherent-state overlap 위에 mode별 thermal Bose occupation `n_k`를 곱한 suppression factor `exp[-1/2 Σ_k (2 n_k + 1)|Δα_k|²]`를 적용하고, branch basis reduced density matrix `ρ`에 대해 일반 Lindblad master equation `dρ/dt = Σ_i γ_i (L_i ρ L_i† - 1/2 {L_i†L_i, ρ})`를 RK4 로 적분한다. 여기에 `bath_spectral_density(ω)`로 mode별 thermal weight 를 재가중하고, `memory_kernel`과 `memory_strength`로 시간 비국소 convolution memory term 을 Lindblad 진화에 더하며, `colored_noise_correlation`의 적분 norm 으로 비백색 환경 잡음의 추가 suppression 을 반영한다. 추가로 `auto_lindblad_from_bath=True`일 때는 branch-basis 에너지 gap과 spectral density, 온도에서 emission/absorption jump operator 를 자동 생성해 KMS detailed-balance ratio `Γ_up/Γ_down = e^{-βΔE}`를 만족하는 surrogate bath coupling 을 구성하고, `detailed_balance_deviation`으로 그 편차를 보고한다. 따라서 기존의 유한 개 mode + Markovian Lindblad surrogate 에서 더 나아가 spectral-density-shaped bath, KMS-consistent thermal transition, non-Markovian/colored-noise surrogate open-system 진화를 직접 모사한다. 다만 완전한 exact open QFT influence functional, 연속 bath 로부터의 self-consistent operator renormalization, 다시간 비국소 메모리 핵의 비가우시안 적분은 아직 남아 있다.
