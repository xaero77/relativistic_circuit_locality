...
- coherent-state overlap 기반 partial trace 에 더해, mode별 thermal occupation(`environment_temperature` 또는 `thermal_occupations`)과 일반 Lindblad jump operator(`lindblad_operators`, `lindblad_rates`, `lindblad_time`)를 포함해 purity/coherence 를 계산하는 `DecoherenceResult`, `compute_decoherence_model` 추가
...
- **decoherence 모델 단순화 (부분 해소)**: `compute_decoherence_model`이 vacuum coherent-state overlap 위에 mode별 thermal Bose occupation `n_k`를 곱한 suppression factor `exp[-1/2 Σ_k (2 n_k + 1)|Δα_k|²]`를 적용하고, branch basis reduced density matrix `ρ`에 대해 일반 Lindblad master equation `dρ/dt = Σ_i γ_i (L_i ρ L_i† - 1/2 {L_i†L_i, ρ})`를 RK4 로 적분한다. `environment_temperature`로 `ω_k = sqrt(|k|² + m²)`에서 thermal occupation 을 자동 생성하거나 `thermal_occupations`를 직접 줄 수 있고, 임의의 jump operator 행렬과 rate/time/step 수를 전달해 dephasing·amplitude damping·population transfer 를 포함한 개방계 진화를 모사할 수 있다. 다만 이는 유한 개 mode와 사용자가 지정한 jump operator에 대한 surrogate open-system 모델이며, 연속 bath spectral density 로부터의 미시적 유도, KMS/Markov 근사 정당화, non-Markovian memory kernel, colored noise 를 포함한 exact open QFT 동역학은 아직 남아 있다.
